...
class AppStrings {
  ...
  // Auth Screen
  String get authTitle => isBengali ? '????????? // ???' : (isHindi ? '????????? // ???' : 'KRISHIMITRA // AI');
  String get authSignInHeader => isBengali ? '?? ?? ????' : (isHindi ? '???? ?? ????' : 'Operator Sign In');
  String get authCreateHeader => isBengali ? '???? ?????????? ???? ????' : (isHindi ? '??? ???? ?????' : 'Create Farmer Account');
  String get authNameLabel => isBengali ? '????? ???? ???' : (isHindi ? '???? ???? ???' : 'FARMER FULL NAME');
  String get authUsernameLabel => isBengali ? '????????' : (isHindi ? '?????????? ???' : 'USERNAME / OPERATOR ID');
  String get authPasswordLabel => isBengali ? '??????????' : (isHindi ? '???????' : 'PASSWORD');
  String get authSignInBtn => isBengali ? '?????? ????' : (isHindi ? '???? ?? ????' : 'SIGN IN TO EDGE APP');
  String get authRegisterBtn => isBengali ? '??????? ????' : (isHindi ? '??????? ????' : 'REGISTER & SYNC PROFILE');
  String get authToggleToRegister => isBengali ? '?????????? ???? ??????? ????' : (isHindi ? '???? ???? ??? ??????? ????' : 'No account yet? Create one.');
  String get authToggleToSignIn => isBengali ? '?????????? ???? ?? ?? ????' : (isHindi ? '???? ??? ???? ?? ????' : 'Already have an account? Sign in.');
